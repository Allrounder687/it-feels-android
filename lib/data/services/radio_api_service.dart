...
class RadioApiService {
  ...
  Future<List<Song>> searchStations(String query, {int limit = 30}) async {
    try {
      final response = await http.get(Uri.parse('\$_baseUrl/stations/search?name=\$query&limit=\$limit'));
      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
        return data.map((json) => _mapStationToSong(json)).toList();
      }
