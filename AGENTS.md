...
- **3-Layer Adaptive Glassmorphism Desktop UI:** 
  - **Layer 1 (Native OS Compositor):** Integrated `flutter_acrylic` to intercept the window render tree natively, injecting an absolute `WindowEffect.acrylic` (Windows) and `WindowEffect.sidebar` (macOS) layer at the compositor level.
  - **Layer 2 (Adaptive Dark Shield):** Engineered `_GlassShieldWrapper` in `main_navigation_wrapper.dart`. This strictly applies a 65% black tint paired with a 30px blur over the entire application scaffolding, ensuring 100% text readability over any wallpaper while retaining the ambient glass effect.
  - **Layer 3 (Glass Container UI):** Created a reusable `GlassContainer` paired with new `themeTextShadow` contrast guards. This overrides standard Material cards with a 5% white fill and 12% white border exclusively on Desktop platforms when the Glass theme is enabled.
